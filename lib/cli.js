...
const loadConf = require("ava/lib/load-config");
...
exports.run = () => {
  ...
  const cli = meow(
	...
	`,
    {
      flags: {
        ...
        renderer: {
          type: "boolean",
          default: conf.renderer
        },
        fixture: {
          type: "string",
          default: conf.fixture
        }
      }
    }
  );
    ...
    del("*", {
      ...
      () => {
        console.error(
          `\n${chalk.green(
            figures.tick
          )} Removed AVA cache files in ${cacheDir}`
        );
        process.exit(0); // eslint-disable-line unicorn/no-process-exit
      },
      error => {
        exit(
          `Error removing AVA cache files in ${cacheDir}\n\n${chalk.gray(
  ...
  const api = new Api({
    ...
    snapshotDir: conf.snapshotDir
      ? path.resolve(projectDir, conf.snapshotDir)
      : null,
    color: conf.color,
    ...
    fixture: cli.flags.fixture || conf.fixture,
  ...
  if (conf.watch) {
    ...
    api.run(files).then(runStatus => {
      process.exitCode = runStatus.suggestExitCode({
        matching: match.length > 0
      });
      reporter.endRun();
    });
  }
